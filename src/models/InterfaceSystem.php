...
class InterfaceSystem extends Model
{
    ...
    public function attributeLabels()
    {
        return [
            ...
            'rule'            => '接口调用规则，inner当前系统，不需要验证和调用',
            'ext'             => '扩展字段数据',
            'is_enable'       => '系统是否启用状态[0:未启用; 1:已启用]，未启用抛异常',
            'is_continue'     => '接口未申明(pub_interfaces)是否继续调用[0:抛异常; 1:继续调用]',
            'is_record_field' => '接口是否记录新字段，调试接口字段[0:不记录，在生成; 1:记录，用于接口调试阶段]',
            'is_open_log'     => '是否开启接口日志[0:未启用; 1:已启用]',
            'sort_order'      => '显示排序',
            'created_at'      => '创建时间',
            'updated_at'      => '更新时间',
        ];
