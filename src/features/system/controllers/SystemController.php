...
class SystemController extends RestController
{
    ...
    public function actionList()
    {
        ...
        $params = $this->validateParams([
            ['alias', 'string', 'label' => '系统标记'],
            ['name', 'string', 'label' => '系统名称'],
            ['is_enable', 'in', 'label' => '是否启用', 'range' => array_keys(TLabelEnable::enableLabels())],
            ['is_continue', 'in', 'label' => '未申明继续', 'range' => array_keys(TLabelYesNo::isLabels())],
            ['is_record_field', 'in', 'label' => '记录新字段', 'range' => array_keys(TLabelYesNo::isLabels())],
            ['is_open_log', 'in', 'label' => '开启接口', 'range' => array_keys(TLabelYesNo::isLabels())],
        ], null, true);
        // 业务处理
    ...
    public function actionAdd()
    {
        // 参数验证和获取
        $params = $this->validateParams([
            [['alias', 'name', 'sort_order'], 'required'],
            ['alias', 'unique', 'label' => '系统别名', 'targetClass' => System::class, 'targetAttribute' => 'alias'],
            ['name', 'unique', 'label' => '系统名称', 'targetClass' => System::class, 'targetAttribute' => 'name'],
            ...
            ['is_enable', 'in', 'range' => array_keys(TLabelEnable::enableLabels()), 'label' => '是否启用'],
            ['is_continue', 'in', 'range' => array_keys(TLabelYesNo::isLabels()), 'label' => '未申明继续调用'],
            ['is_record_field', 'in', 'range' => array_keys(TLabelYesNo::isLabels()), 'label' => '记录新字段'],
            ['is_open_log', 'in', 'range' => array_keys(TLabelYesNo::isLabels()), 'label' => '开启接口'],
        ]);
        // 业务处理
